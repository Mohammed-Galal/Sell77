<div class="__inline-9 d-none d-sm-block rtl">
...
const hostName = window.location.hostname,
    ...
    links = {},
    mobileView = window.matchMedia("(max-width: 576px)");

links.home = {href: "/", icon: "tio-home"};
links.search = {href: "/products", icon: "tio-search"};
links.cart = {href: "/shop-cart", icon: "czi-cart"};
links.account = {href:  "/user-account", icon: `tio-user-big`};

const navList = document.createElement("ul"),
    navItems = Object.keys(links).map(navItem);

mobileNav.style = `position: sticky;bottom: 0px;background-color: rgb(255, 255, 255);width: 100%;z-index: 1000000;text-transform: capitalize;font-weight: 600;`;
mobileNav.className = "py-1 d-block d-sm-none";
navList.style.setProperty("--size", "48px");
navList.className = "container d-flex gap-2 list-unstyled my-0";
...
navList.append.apply(navList, navItems);

mobileView.onchange = handleView;
function handleView(params) {
    let navHeight = 0;
    if (params.matches) {
        document.body.classList.add("p-0");
        navHeight = mobileNav.scrollHeight;
    }
    docEl.style.setProperty("--mobile-nav", navHeight + "px");
}
handleView({matches: window.innerWidth < 576});

function navItem(itemName){
    const {href, icon} = links[itemName],
        li = document.createElement("li"),
        a = document.createElement("a"),
        i = document.createElement("i");

    const isActive = new RegExp("^" + href + "/?$").test(window.location.pathname);
    i.style = "width: var(--size);height: var(--size);background: #f3f5f9;clip-path: circle(50%);font-size: 1.3rem";
    i.className = "align-items-center d-flex justify-content-center " + icon;
    a.href = window.location.protocol+ "//" + hostName + href;
    a.style = "color: var(--web-primary);font-size: small;";
    a.className = "align-items-center d-flex flex-column gap-1";
    li.className = "m-0";
    ...
    if (isActive) {
        i.style.backgroundColor = "var(--web-primary)";
        i.style.color = "#fff";
    }
    return li;
}
